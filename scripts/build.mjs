...
// These essays have intentionally handcrafted pages in public/ and should not
// be regenerated by this script. Keep their metadata here so the shared home,
// essay index, library and sitemap still know they exist.
const supplementalEssays = [
  {
    slug: 'maya',
    title: 'MAYA',
    description: 'A memory from Tirupati, a restaurant called Maya, and a question about how much of life we spend looking outward rather than within.',
    date: '2026-09-11',
    readingTime: '3 min read',
    tags: ['Looking Inward', 'Attention', 'Reflection'],
    collection: 'Looking Inward'
  },
  {
    slug: 'when-the-data-is-right-but-the-comparison-is-wrong',
    title: 'When the Data Is Right, but the Comparison Is Wrong',
    description: 'A reflection on data, comparisons and why human judgment matters even more when information becomes abundant.',
    date: '2026-09-01',
    readingTime: '2 min read',
    tags: ['Data', 'Judgment', 'AI'],
    collection: 'Through My Quiet Lens'
  }
];
const listedEssays = [...essays, ...supplementalEssays];
...
const homeLatest = [
  ...listedEssays.map(e => ({ ...e, _cat: 'essays' })),
...
const libraryAllItems = [...listedEssays.map(e=>({...e,url:`/essays/${e.slug}/`})),...principles.map(e=>({...e,collection:'Working Principles',url:`/principles/${e.slug}/`,description:e.text})),...dc.map(e=>({...e,collection:'DC Decoded',url:`/dc-decoded/${e.slug}/`}))];
...
const standaloneEssays = listedEssays
  .filter(e=>e.collection!=='Through My Quiet Lens')
  .sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
