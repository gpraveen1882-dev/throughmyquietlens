...
const prettyDate = (d) => new Intl.DateTimeFormat('en-GB', { day:'numeric', month:'long', year:'numeric' }).format(new Date(`${d}T12:00:00Z`));
...
<section class="section"><div class="container"><div class="section-head"><div><p class="eyebrow">Featured essay</p><h2>Begin here</h2></div><p>A considered starting point for understanding the ideas that shape this publication.</p></div><article class="featured"><div><div class="meta"><span>${prettyDate(featured.date)}</span><span>${featured.readingTime}</span></div><h3><a href="/essays/${featured.slug}/">${esc(featured.title)}</a></h3></div><div><p>${esc(featured.description)}</p>${tagHtml(featured.tags)}<p><a class="read-link" href="/essays/${featured.slug}/">Read the essay →</a></p></div></article></div></section>
<section class="section"><div class="container"><div class="section-head"><div><p class="eyebrow">Recent writing</p><h2>Ideas in progress</h2></div><a href="/essays/">View all essays →</a></div><div class="cards">${recent.map(e=>`<article class="card"><div class="meta"><span>${prettyDate(e.date)}</span><span>${e.readingTime}</span></div><h3><a href="/essays/${e.slug}/">${esc(e.title)}</a></h3><p>${esc(e.description)}</p><a class="read-link" href="/essays/${e.slug}/">Continue reading →</a></article>`).join('')}</div></div></section>
...
const essaysList = `<section class="page-hero"><div class="container"><p class="eyebrow">Writing</p><h1 class="page-title">Essays</h1><p class="page-lead">Stories and reflections that explore an idea rather than rush towards a conclusion.</p></div></section><section class="section"><div class="container list">${essays.map(e=>`<article class="list-item"><div class="meta"><span>${prettyDate(e.date)}</span><span>${e.readingTime}</span></div><div><h2><a href="/essays/${e.slug}/">${esc(e.title)}</a></h2><p>${esc(e.description)}</p>${tagHtml(e.tags)}</div><a href="/essays/${e.slug}/" aria-label="Read ${esc(e.title)}">Read →</a></article>`).join('')}</div></section>`;
...
for (const e of essays) {
  ...
  const body = `<article><header class="article-head"><div class="reading"><p class="eyebrow">Essay</p><h1>${esc(e.title)}</h1><p class="deck">${esc(e.description)}</p><div class="meta"><span>${prettyDate(e.date)}</span><span>${e.readingTime}</span></div>${tagHtml(e.tags)}</div></header><div class="article reading">${e.body.map(p=>`<p>${esc(p)}</p>`).join('')}<div class="article-end"><strong>Continue exploring</strong>${related.map(r=>`<p><a href="/essays/${r.slug}/">${esc(r.title)} →</a></p>`).join('')}</div></div></article>`;
...
for (const e of dc) {
  const body = `<article><header class="article-head"><div class="reading"><p class="eyebrow">DC Decoded</p><h1>${esc(e.title)}</h1><p class="deck">${esc(e.description)}</p><div class="meta"><span>${prettyDate(e.date)}</span><span>${e.readingTime}</span></div></div></header><div class="article reading">${e.body.map(p=>`<p>${esc(p)}</p>`).join('')}<div class="article-end"><a href="/dc-decoded/">Back to DC Decoded →</a></div></div></article>`;
...
const rssItems = essays.map(e=>`<item><title>${esc(e.title)}</title><link>${site.url}/essays/${e.slug}/</link><guid>${site.url}/essays/${e.slug}/</guid><pubDate>${new Date(`${e.date}T12:00:00Z`).toUTCString()}</pubDate><description>${esc(e.description)}</description></item>`).join('');
